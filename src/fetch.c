...
static int usage(void) {
  ms_help(stderr,
    ...
    ".cg example fixtures (GitHub methscope_data) into the local store.\n\n"
    "With no NAME: a full-screen picker on a terminal (arrows/jk move, space\n"
    "toggles, a/n all/none, / filters, f fetches, q or Esc cancels), and a plain\n"
    "listing everywhere else. A named target never prompts, so a build or a\n"
    "workflow step can never wait for input.\n\n"
