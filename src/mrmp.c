  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (!strcmp(a, "-h") || !strcmp(a, "--help")) {
      ms_help(stderr,
        ...
        "  --qfilter LO,HI           (default 0.30,0.70)\n"
...
/* ======================================================================== */
/* mrmp-planes: per-class q-filter state as two genome-wide bit planes.      */
/* ======================================================================== */
/* Rebuilding an MRMP over a SUBSET of classes is the operation that pays. A
 * pattern must hold across every class in its set, so the q-filter is a
 ...
 * 2-class set -- and 92% of that gap is CpGs the wider filter REJECTED, not
 * CpGs it merely scattered. Deleting bits from binstrings and re-merging
 * reaches none of it, which is why that arm scored below doing nothing.
 *
 * The blocker is cost: a per-subset rebuild reads the store three times (bit
 * planes, selection, thresholds). This persists what the first two compute.
 *
 * Per (class, CpG) exactly two bits, and both are properties of that class
 * ALONE -- neither depends on which other classes are in the set:
 *
 *     side   the binstring bit, beta > --beta-threshold
 *     ok     covered AND clearing the q-filter ON ITS OWN SIDE, i.e.
 *            side ? beta >= --qfilter-hi : beta <= --qfilter-lo
 *
 * giving the four states 0 / 1 / 0f / 1f. Then for any class set S:
 *
 *     admissible(i,S) = AND over k in S of ok[k][i]
 *     binstring(i,S)  = the side bits of S, constant ones dropped
 *
 * This is EXACT, not an approximation, and rests on three properties of the
 * settings it is valid for:
 *
 *  - --max-frac-na 0 rejects a CpG where ANY class in S is absent, so a
 *    surviving CpG has every class covered. resolve_cpg's sweeping-majority
 *    FILL -- the one genuinely set-dependent step, since it reads the majority
 *    over the set -- therefore never fires on a survivor, and the binstring
 *    collapses to the side bits.
 *  - --delta-mean-top 0 (uncapped) makes admission the whole selection. A
 *    capped build ranks by delta_mean, which needs betas.
 *  - --depth-floor-frac 0 and --min-cg-depth 0: both add depth tests the ok bit
 *    does not carry.
 *
 * NOT covered: per-pattern binarisation midpoints (MRMP_FLAG_THRESH) need each
 * class's MEAN BETA over a pattern, which bits cannot give -- that is the third
 * store pass and it remains. Removing it needs per-ATOM (distinct state vector)
 * sums, so this reports the atom count: the number that decides whether such a
 * table is megabytes or gigabytes. */

#define MSPLANE_MAGIC   "MSPLANE1"
#define MSPLANE_VERSION 1u

/* Header, then the NUL-terminated reference path, the n_class NUL-terminated
 * names, then the side plane and the ok plane. Both planes are CLASS-MAJOR in
 * groups of 8 -- byte g*n_cpg + i holds CpG i for classes 8g..8g+7 -- which is
 * the layout mrmp.c already builds them in, and the one the operation wants: a
 * 6-class set touches only its own groups instead of streaming all 33. */
typedef struct {
  char     magic[8];
  uint32_t version;
  uint32_t n_class;
  uint64_t n_cpg;
  uint32_t stride;              /* (n_class + 7) >> 3 bytes per CpG per plane */
  uint32_t mincov;
  float    beta_threshold;
  float    qfilter_lo, qfilter_hi;
  uint64_t n_atoms;             /* distinct (side, ok) vectors; 0 = not counted */
  uint64_t refname_offset;
  uint64_t names_offset;
  uint64_t side_offset;
  uint64_t ok_offset;
} msplane_header_t;

/* Open-addressed set of atom HASHES. An exact count would need the vectors
 * themselves; the number is used to size a table, not to key one, so a rare
 * 64-bit collision costs nothing that matters here. */
typedef struct { uint64_t *h; uint64_t mask, n; } atomset_t;

static uint64_t plane_fnv64(const uint8_t *p, size_t n) {
  uint64_t x = 1469598103934665603ULL;
  for (size_t i = 0; i < n; ++i) { x ^= p[i]; x *= 1099511628211ULL; }
  return x;
}

static void aset_add(atomset_t *a, uint64_t hash) {
  if (!hash) hash = 1;
  uint64_t i = hash & a->mask;
  while (a->h[i]) {
    if (a->h[i] == hash) return;
    i = (i + 1) & a->mask;
  }
  a->h[i] = hash; a->n++;
  if (a->n * 2 > a->mask) {                          /* grow at 50% load */
    uint64_t old_mask = a->mask, *old = a->h;
    a->mask = a->mask * 2 + 1;
    a->h = xcalloc(a->mask + 1, sizeof(uint64_t), "atom set grow");
    for (uint64_t j = 0; j <= old_mask; ++j) {
      if (!old[j]) continue;
      uint64_t k = old[j] & a->mask;
      while (a->h[k]) k = (k + 1) & a->mask;
      a->h[k] = old[j];
    }
    free(old);
  }
...
int main_mrmp_planes(int argc, char *argv[]) {
  ...
  float qlo = 0.25f, qhi = 0.60f, beta_thr = MRMP_DEF_BETA_THRESH;
  uint32_t mincov = MRMP_DEF_MINCOV;
  int count_atoms = 1, force = 0;
  ...
  for (; i < argc; ++i) {
    ...
    if (!strcmp(a, "-o") && i + 1 < argc) out_path = argv[++i];
      ...
      if (sscanf(argv[++i], "%f,%f", &qlo, &qhi) != 2)
        die("--qfilter expects LO,HI", argv[i]);
    }
    else if (!strcmp(a, "--beta-threshold") && i + 1 < argc)
      beta_thr = (float)atof(argv[++i]);
    else if (!strcmp(a, "--mincov") && i + 1 < argc)
      mincov = (uint32_t)parse_u64(argv[++i], a);
    else if (!strcmp(a, "--no-atom-count")) count_atoms = 0;
    else if (!strcmp(a, "--force")) force = 1;
    else if (!strcmp(a, "-h") || !strcmp(a, "--help")) {
      printf(
"Usage: methscope mrmp-planes [options] REF.cg -o OUT.msplane\n"
"\n"
"  Persist the per-class q-filter state of every genome CpG as two bit\n"
"  planes, so an MRMP can be rebuilt over any SUBSET of the classes without\n"
"  re-reading the reference store.\n"
"\n"
"  Rebuilding over a subset is the operation that pays: a pattern must hold\n"
"  across every class in its set, so dropping classes admits CpGs the wider\n"
"  filter rejected. Monocyte vs Macrophage carries 300 segregating CpGs in a\n"
"  33-class build and 11,935 as its own 2-class set, and 92%% of that gap is\n"
"  rescued CpGs rather than regrouped ones -- merging binstrings gets none.\n"
"\n"
"  Two bits per (class, CpG), the states 0 / 1 / 0f / 1f:\n"
"    side   the binstring bit, beta > --beta-threshold\n"
"    ok     covered AND clearing the q-filter on its own side\n"
"  Both are properties of the class alone, so for any class set S\n"
"  admissibility is an AND of the ok planes of S and the binstring is the\n"
"  side bits. No store access, no betas.\n"
"\n"
"  Options\n"
"    -o OUT.msplane        output (required)\n"
"    --qfilter LO,HI       the band the ok bit records (default 0.30,0.70).\n"
"                          BAKED IN: the thresholds in use are not nested\n"
"                          (0.30 is looser on the 0 side, 0.70 stricter on\n"
"                          the 1 side), so one artifact cannot serve both.\n"
"    --beta-threshold B    binstring cut for the side bit (default 0.50)\n"
"    --mincov N            min per-class coverage (default 1)\n"
"    --no-atom-count       skip counting distinct state vectors\n"
"    --force               overwrite an existing output\n"
"\n"
"  Valid only for builds with --max-frac-na 0, --delta-mean-top 0,\n"
"  --depth-floor-frac 0 and --min-cg-depth 0; the rest need betas.\n");
      return 0;
    }
    else if (a[0] == '-' && a[1]) die("unrecognized option", a);
    else break;
  }
  ...
  if (!force) {
    ...
    if (t) { fclose(t); die("output exists (use --force)", out_path); }
  }

  uint32_t ns = 0;
  int64_t *voff = NULL;
  char **label = read_store_index(store, &ns, &voff);

  const uint32_t stride = (ns + 7) >> 3;
  uint64_t n_cpg = 0;
  uint8_t *side = NULL, *ok = NULL;
  ...
  for (uint32_t k = 0; k < ns; ++k) {
    ...
    if (!k) {
      n_cpg = c.n;
      side = xcalloc((size_t)stride * n_cpg, 1, "side plane");
      ok   = xcalloc((size_t)stride * n_cpg, 1, "ok plane");
    } else if (c.n != n_cpg) {
      die("store records disagree on CpG count", label[k]);
    }
    const uint64_t base = (uint64_t)(k >> 3) * n_cpg;
    const uint8_t bit = (uint8_t)(1u << (k & 7));
    for (uint64_t j = 0; j < n_cpg; ++j) {
      uint64_t mu = f3_get_mu(&c, j);
      if (!mu || MU2cov(mu) < mincov) continue;    /* absent: side 0, ok 0 */
      double b = MU2beta(mu);
      /* M==U is not a call; mrmp.c treats it as ambiguous, and an ambiguous
       * class is absent for our purposes, so leave both bits clear. */
      if (b == (double)beta_thr) continue;
      if (b > (double)beta_thr) {
        side[base + j] |= bit;
        if (b >= (double)qhi) ok[base + j] |= bit;
      } else if (b <= (double)qlo) {
        ok[base + j] |= bit;
      }
    }
    free_cdata(&c);
    fprintf(stderr, "\r[methscope] mrmp-planes: %u/%u classes", k + 1, ns);
    fflush(stderr);
  }
  bgzf_close(cf.fh);
  fputc('\n', stderr);
  ...
  if (count_atoms) {
    atomset_t as;
    as.n = 0; as.mask = (1ull << 20) - 1;
    as.h = xcalloc(as.mask + 1, sizeof(uint64_t), "atom set");
    uint8_t *vec = xcalloc((size_t)2 * stride, 1, "atom vector");
    for (uint64_t j = 0; j < n_cpg; ++j) {
      for (uint32_t g = 0; g < stride; ++g) {
        vec[g]          = side[(uint64_t)g * n_cpg + j];
        vec[stride + g] = ok[(uint64_t)g * n_cpg + j];
      }
      aset_add(&as, plane_fnv64(vec, (size_t)2 * stride));
    }
    n_atoms = as.n;
    free(vec); free(as.h);
  }

  msplane_header_t hd; memset(&hd, 0, sizeof hd);
  memcpy(hd.magic, MSPLANE_MAGIC, 8);
  hd.version = MSPLANE_VERSION; hd.n_class = ns; hd.n_cpg = n_cpg;
  hd.stride = stride; hd.mincov = mincov; hd.beta_threshold = beta_thr;
  hd.qfilter_lo = qlo; hd.qfilter_hi = qhi; hd.n_atoms = n_atoms;

  uint64_t off = sizeof hd;
  hd.refname_offset = off; off += strlen(store) + 1;
  hd.names_offset = off;
  for (uint32_t s = 0; s < ns; ++s) off += strlen(label[s]) + 1;
  off = (off + 7u) & ~7ull;
  hd.side_offset = off;    off += (uint64_t)stride * n_cpg;
  hd.ok_offset = off;

  FILE *fp = fopen(out_path, "wb");
  if (!fp) die("cannot open output", out_path);
  if (fwrite(&hd, sizeof hd, 1, fp) != 1) die("write failed", out_path);
  fwrite(store, 1, strlen(store) + 1, fp);
  for (uint32_t s = 0; s < ns; ++s)
    fwrite(label[s], 1, strlen(label[s]) + 1, fp);
  long pad = (long)hd.side_offset - ftell(fp);
  for (long p = 0; p < pad; ++p) fputc(0, fp);
  size_t plane_bytes = (size_t)stride * n_cpg;
  if (fwrite(side, 1, plane_bytes, fp) != plane_bytes)
    die("write failed (side plane)", out_path);
  if (fwrite(ok, 1, plane_bytes, fp) != plane_bytes)
    die("write failed (ok plane)", out_path);
  fclose(fp);

  fprintf(stderr,
    "[methscope] mrmp-planes: %u classes x %llu CpGs, %u B/CpG/plane\n"
    "            qfilter %.2f,%.2f  beta-threshold %.2f  mincov %u\n"
    "            %.1f MB -> %s\n",
    ns, (unsigned long long)n_cpg, stride, qlo, qhi, beta_thr, mincov,
    (double)(2.0 * plane_bytes + sizeof hd) / 1e6, out_path);
  if (count_atoms)
    fprintf(stderr,
      "            %llu distinct state vectors (atoms); a per-atom x class\n"
      "            sum/count table would be %.1f MB\n",
      (unsigned long long)n_atoms, (double)n_atoms * ns * 12.0 / 1e6);

  free(side); free(ok);
  for (uint32_t s = 0; s < ns; ++s) free(label[s]);
  free(label); free(voff);
  return 0;
}
