...
/**
 * deconv2: a packed reference for per-query deconvolution.
 *
 * WHAT THIS REPLACES
 *
 ...
 *   betas     2 * n_class * n_kept   uint16, CLASS-MAJOR
 *
...
static void d2die(const char *msg, const char *arg) {
  if (arg) fprintf(stderr, "[methscope] deconv2-build-ref: %s: %s\n", msg, arg);
  else     fprintf(stderr, "[methscope] deconv2-build-ref: %s\n", msg);
  exit(1);
}
...
int main_deconv2_build_ref(int argc, char *argv[]) {
  ...
  for (; i < argc; ++i) {
    ...
    if (!strcmp(a, "-o") && i + 1 < argc) out_path = argv[++i];
      ...
      printf(
"Usage: methscope deconv2-build-ref [options] <celltypes.cg> -o <out.msdref>\n"
"\n"
"  Pack a per-cell-type M/U store into the uint16 beta reference the deconv2\n"
...
"    --force            overwrite an existing output\n");
  ...
  if (argc - i != 1 || !out_path) {
    fprintf(stderr,
      "Usage: methscope deconv2-build-ref <celltypes.cg> -o <out.msdref>\n");
    return 1;
  }
  ...
  for (uint32_t k = 0; k < n_class; ++k) {
    ...
    free_cdata(&c);
    fprintf(stderr, "\r[methscope] deconv2-build-ref: reading %u/%u",
            k + 1, n_class);
    fflush(stderr);
  }
  bgzf_close(cf.fh);
  ...
  for (uint32_t k = 0; k < n_class; ++k) {
    const uint16_t *src = beta + (size_t)k * n_row;
  ...
  fprintf(stderr,
    "[methscope] deconv2-build-ref: %u classes x %llu rows -> %llu kept "
    "(%.2f%%)\n"
  ...
  free(buf); free(keep); free(beta); free(voff);
...
typedef struct {
  ...
  uint16_t *beta;             /* n_class * n_keep, class-major */
} d2ref_t;
...
static void d2ref_load(const char *path, d2ref_t *R) {
  ...
  if (ver != 1) d2die("unsupported .msdref version", path);
  ...
  R->beta = d2alloc((size_t)R->n_class * R->n_keep, sizeof(uint16_t), "betas");
  if (fread(R->row, sizeof(uint32_t), R->n_keep, f) != R->n_keep)
    d2die("truncated row index", path);
  if (fread(R->beta, sizeof(uint16_t), (size_t)R->n_class * R->n_keep, f)
      != (size_t)R->n_class * R->n_keep)
    d2die("truncated beta block", path);
  fclose(f);
  fprintf(stderr,
    "[methscope] deconv2: reference %u classes x %llu kept rows "
    "(%.0f MB), qfilter %.2f,%.2f\n",
    R->n_class, (unsigned long long)R->n_keep,
    (double)((size_t)R->n_class * R->n_keep * 2) / 1e6, R->qlo, R->qhi);
}

static void d2ref_free(d2ref_t *R) {
  for (uint32_t k = 0; k < R->n_class; ++k) free(R->name[k]);
  free(R->name); free(R->row); free(R->beta);
}
...
static void d2pan_build(const d2ref_t *R, const d2q_t *Q,
  ...
  for (uint64_t w = 0; w < Q->nw; ++w) {
    ...
    for (uint32_t s = 0; s < ns; ++s) {
      uint16_t b = R->beta[(size_t)sel[s] * R->n_keep + j];
    ...
    if (!global_ref) {
      ...
      for (uint32_t c = 0; c < out->nall; ++c) {
        uint16_t b = R->beta[(size_t)c * R->n_keep + j];
  ...
  if (dm_top) {
    ...
    for (size_t a = 0; a < n_ent; ) {
      ...
      for (size_t z = 0; z < take; ++z) {
        ...
        double *rs = out->rsum + (size_t)r * out->nall;
        for (uint32_t c = 0; c < out->nall; ++c) {
          uint16_t bb = R->beta[(size_t)c * R->n_keep + j];
...
static void d2_solve(const d2pan_t *p, const uint32_t *sel, uint32_t ns,
  ...
  if (mode != 1)
    fprintf(stderr, "[methscope] deconv2: NNLS mode %d (%u patterns)\n",
            mode, nu);
  double sum = 0;
...
static void d2ws_free(d2ws_t *w) {
  ...
  free(w->inscope); free(w->inscope2); free(w->done);
  free(w->x); free(w->x2); free(w->xf); free(w->xadj);
}

typedef struct {
  uint64_t min_n, dm_top, sat_dm_top, max_seg;
...
static void d2_union_sats(const d2ref_t *R, const d2q_t *Q, const d2pan_t *P,
  ...
  for (uint32_t i = 0; i < ns; ++i)
    for (uint32_t j = i + 1; j < ns; ++j) {
      ...
      uint32_t pr[2] = { ga, gb };
      d2pan_t A; d2pan_build(R, Q, pr, 2, sat_dm, global_ref, &A);
...
static void d2_record(const d2ref_t *Rr, const d2opt_t *o, d2ws_t *w,
  ...
  (void)segu;
      ...
      *comma = '\0';
      ...
      for (uint64_t w = 0; w < Q.nw; ++w) {
        ...
        while (bits) {
          const uint64_t j = (w << 6) + (uint64_t)__builtin_ctzll(bits);
          bits &= bits - 1;
          uint16_t ba = R.beta[(size_t)ia * R.n_keep + j];
          uint16_t bb = R.beta[(size_t)ib * R.n_keep + j];
          ...
          for (uint32_t s2 = 0; s2 < R.n_class && all_ok; ++s2) {
            uint16_t b2 = R.beta[(size_t)s2 * R.n_keep + j];
      ...
      fprintf(stderr,
        "[methscope] deconv2: %s: %s vs %s -- %llu measured CpGs segregate the "
    ...
    d2pan_t P; d2pan_build(&R, &Q, sel, R.n_class, dm_top, global_ref, &P);
    ...
    if (verbose) {
      ...
      fprintf(stderr,
        "[methscope] deconv2: %s: %llu measured rows -> %u patterns over "
        "%llu CpGs\n",
    ...
    }
    {
      ...
      if (verbose) {
        fprintf(stderr, "[methscope] deconv2: %s: scope %u/%u ->",
      ...
      for (uint32_t round = 2; narrow && !o->force_scope && round <= max_round;
        ...
        d2pan_t P2; d2pan_build(&R, &Q, sel2, n_sel2, dm_top, global_ref, &P2);
        d2_seg_matrix(&R, &P2, sel2, n_sel2, seg2);
        ...
        if (verbose) {
          ...
          fprintf(stderr,
            "[methscope] deconv2: %s: round %u on %u classes -> %u patterns "
     ...
     * global one instead. */
    {
      ...
      if (n_fin >= 2) {
        d2pan_t PF; d2pan_build(&R, &Q, sel2, n_fin, dm_top, global_ref, &PF);
...
int main_deconv2(int argc, char *argv[]) {
  ...
  uint64_t min_n = 5, dm_top = 0, sat_dm_top = 0;
  ...
  for (; i < argc; ++i) {
    ...
    if (!strcmp(a, "-o") && i + 1 < argc) out_path = argv[++i];
      ...
      printf(
"Usage: methscope deconv2 [options] <query.cg> <ref.msdref> -o <out.tsv>\n"
"\n"
...
"\n"
"  The measured-row restriction happens first and is structural: a pattern's\n"
"  reference beta and its observed beta are means over the same CpGs, so the\n"
"  two sides of each equation are comparable and the CpG count that sets the\n"
"  row weight is the honest support of both.\n"
"\n"
"  Options\n"
...
"                   separators rather than everything admissible.\n"
"    --max-segregating N  two classes separated by <= N MEASURED CpGs are\n"
"                   inseparable for this query, so if either carries mass\n"
...
"                   in the design matrix attenuates coefficients toward zero,\n"
"                   so a noisier column loses mass to a cleaner correlated\n"
"                   one; the legacy .refx has no such noise because its design\n"
"                   values were computed once at build time. Costs a second\n"
...
"    --panel-out F  dump the rebuilt panel: binstring, measured CpGs,\n"
"                   observed beta -- one row per pattern per record\n"
"    -v             per-record panel size and measured-row count\n");
      return 0;
    }
    else if (a[0] == '-' && a[1]) d2die("unrecognized option", a);
    else break;
  ...
  if (argc - i != 2) {
    fprintf(stderr,
      "Usage: methscope deconv2 <query.cg> <ref.msdref> -o <out.tsv>\n");
    return 1;
  }
  ...
  if (nthreads > 1 && (verbose || panel_out || scope_out)) {
    fprintf(stderr, "[methscope] deconv2: -v/--panel-out/--scope-out are "
                    "single-threaded; using 1 thread\n");
    nthreads = 1;
  }
  if (nthreads > 1 && !n_qname) {
    fprintf(stderr, "[methscope] deconv2: no <query>.idx to seek by; "
                    "using 1 thread\n");
    nthreads = 1;
  }
