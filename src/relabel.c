...
int main_relabel(int argc, char *argv[]) {
  ...
  for (; i < argc; ++i) {
    ...
    if (!strcmp(a, "-o") && i + 1 < argc) out_path = argv[++i];
      ...
      printf(
"Usage: methscope relabel [options] OLD=NEW IN.clfx -o OUT.clfx\n"
"\n"
"  Rename a class label inside a trained model. The label lives in one\n"
"  XGBoost booster attribute (\"methscope_labels\", comma-separated in\n"
"  class-index order), so this rewrites that string and nothing else --\n"
"  every prediction is bit-identical and only the emitted name changes.\n"
"\n"
"  A class name is a claim about what the reference CONTAINS, and it can be\n"
"  wrong while the model is right. Retraining to correct a name would\n"
"  perturb numbers that were never at fault.\n"
"\n"
"  On a TREE bundle every node carrying the class has its own copy of the\n"
"  label list, so all of them are walked; renaming the root alone would\n"
"  leave the leaves disagreeing with it.\n"
"\n"
"  The bundled MRMP's reference sample names are left ALONE on purpose:\n"
"  they record what was built, not what it is.\n"
"\n"
"  Options\n"
"    -o OUT.clfx   output (required; the input is never modified)\n"
