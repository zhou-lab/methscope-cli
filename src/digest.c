 ...
 *
 * `fetch` verifies every download against a digest compiled into the registry,
 * so a corrupted, truncated, or substituted file is rejected before it is
 * installed. OpenSSL would be a whole dependency for one hash function and
 * CommonCrypto is macOS-only; this keeps the bioconda surface at
 * htslib + zlib + libcurl. Ported from kycg's src/digest.c, which solves the
 * identical problem for the annotation repository. */
#include <stdint.h>
#include <stdio.h>
#include <string.h>
