...
static uint32_t targets(const MsurHeader*h,const uint8_t*base,const uint32_t*cpg,uint64_t begin,uint32_t O,uint32_t cell,uint32_t rep,uint64_t start,uint32_t want,std::vector<uint32_t>&id,std::vector<float>&y){
  const uint8_t*rec=msur_record(base,h,rep,cell);const uint8_t*sel=msur_observed_set(rec,h);uint32_t n_sel=msur_sample_of(base,h,rep),enc=msur_encoding_of(base,h,rep);const uint16_t*truth=(const uint16_t*)(base+h->truth_offset)+(size_t)cell*h->n_cpg;uint32_t n=0;uint64_t seen=0,q=start%O;
  while(n<want&&seen<O){uint32_t pos=cpg[begin+q];uint16_t v=truth[pos];if(v!=UINT16_MAX&&!msur_observed(sel,enc,n_sel,pos)){id[n]=(uint32_t)q;y[n]=(float)v/65534.0f/* u16 code / 65534; 65535 (UINT16_MAX) = missing */;++n;}if(++q==O)q=0;++seen;}return n;
}
...
extern "C" int ms_upunit_train_cuda(const ms_upunit_config_t*c){
  ...
  if(c->feature_mode>MS_UPFEATURE_SCALAR)die("invalid feature mode");
  int F=(int)ms_upfeature_dim(c->feature_mode,P),I=F,H=0;Map trunk={-1,0,NULL};const UpfacHeader*th=NULL;const float*trunk_par=NULL;uint64_t trunk_floats=0,trunk_checksum=0;
  if(c->trunk_path){
  ...
  /* Cell split: a curated --split file, else cells shuffled by seed and cut
   * 70/15/15. All simulations of a cell stay together either way. Only the
   * explicit labels enter the run checksum, so seeded-split work directories
   * built before --split existed still resume. */
  std::vector<uint32_t>train,val,test;
  if(c->split_path){
    std::vector<uint8_t>label(h->n_cells);ms_upsplit_load("upscale-train",c->split_path,h->n_cells,label.data());
    for(uint32_t i=0;i<h->n_cells;++i)(label[i]==MS_UPSPLIT_TRAIN?train:label[i]==MS_UPSPLIT_VAL?val:test).push_back(i);
    runsum=fnv(runsum,label.data(),label.size());
  }else{
  ...
  if(th){
    ...
    for(uint32_t r=0;r<h->n_reps;++r)for(uint32_t cell:train){const uint8_t*rec=msur_record(data.p,h,r,cell);const float*b=(const float*)rec;const uint32_t*n=(const uint32_t*)(rec+(size_t)h->n_patterns*4);
    if(c->feature_mode==MS_UPFEATURE_SCALAR){double total=0;for(uint32_t p=0;p<P;++p)if(std::isfinite(b[p])){sum[p]+=b[p];cnt[p]++;total+=n[p];}sum[P]+=std::log1p(total);cnt[P]++;continue;}
    for(uint32_t p=0;p<P;++p){uint32_t j=c->feature_mode==MS_UPFEATURE_BETA?p:2*p;if(std::isfinite(b[p])){sum[j]+=b[p];cnt[j]++;}if(c->feature_mode!=MS_UPFEATURE_BETA){double a=c->feature_mode==MS_UPFEATURE_COUNT?std::log1p((double)n[p]):(std::isfinite(b[p])?0.0:1.0);sum[j+1]+=a;cnt[j+1]++;}}}
    /* Feature-major, like the CPU path: j runs over all F inputs. The old
     * per-pattern form indexed j=2*p, which overruns mean[] under SCALAR
     * (F=P+1, not 2P) -- a heap overflow at p=P/2. */
    for(int j=0;j<F;++j){if(!cnt[j])die("one MRMP is always missing in training");mean[j]=(float)(sum[j]/cnt[j]);}
    for(uint32_t r=0;r<h->n_reps;++r)for(uint32_t cell:train){const uint8_t*rec=msur_record(data.p,h,r,cell);const float*b=(const float*)rec;const uint32_t*n=(const uint32_t*)(rec+(size_t)h->n_patterns*4);
    if(c->feature_mode==MS_UPFEATURE_SCALAR){double total=0;for(uint32_t p=0;p<P;++p)if(std::isfinite(b[p])){double d=b[p]-mean[p];ss[p]+=d*d;total+=n[p];}double d=std::log1p(total)-mean[P];ss[P]+=d*d;continue;}
    for(uint32_t p=0;p<P;++p){uint32_t j=c->feature_mode==MS_UPFEATURE_BETA?p:2*p;if(std::isfinite(b[p])){double d=b[p]-mean[j];ss[j]+=d*d;}if(c->feature_mode!=MS_UPFEATURE_BETA){double a=c->feature_mode==MS_UPFEATURE_COUNT?std::log1p((double)n[p]):(std::isfinite(b[p])?0.0:1.0),d=a-mean[j+1];ss[j+1]+=d*d;}}}
  ...
  std::vector<float>X((size_t)rows*F);for(size_t r=0;r<rows;++r){const uint8_t*rec=msur_record(data.p,h,(uint32_t)(r/h->n_cells),(uint32_t)(r%h->n_cells));const float*b=(const float*)rec;const uint32_t*n=(const uint32_t*)(rec+(size_t)h->n_patterns*4);float*x=X.data()+r*F;
    if(c->feature_mode==MS_UPFEATURE_SCALAR){double total=0;for(uint32_t p=0;p<P;++p){x[p]=std::isfinite(b[p])?(b[p]-mean[p])/scale[p]:0;if(std::isfinite(b[p]))total+=n[p];}x[P]=(float)((std::log1p(total)-mean[P])/scale[P]);continue;}
    for(uint32_t p=0;p<P;++p){uint32_t j=c->feature_mode==MS_UPFEATURE_BETA?p:2*p;x[j]=std::isfinite(b[p])?(b[p]-mean[j])/scale[j]:0;if(c->feature_mode!=MS_UPFEATURE_BETA){double a=c->feature_mode==MS_UPFEATURE_COUNT?std::log1p((double)n[p]):(std::isfinite(b[p])?0.0:1.0);x[j+1]=(a-mean[j+1])/scale[j+1];}}}
  CU(cudaSetDevice(c->device));cudaDeviceProp prop;CU(cudaGetDeviceProperties(&prop,c->device));std::fprintf(stderr,"[methscope] upscale-train: GPU %s %.0f MiB; cells=%u reps=%u patterns=%u features=%s external=%d trunk=%d unit_input=%d units=%u split=%zu/%zu/%zu\n",prop.name,(double)prop.totalGlobalMem/1048576,h->n_cells,h->n_reps,P,c->feature_mode==MS_UPFEATURE_COUNT?"beta+count":c->feature_mode==MS_UPFEATURE_MISSING?"beta+missing":c->feature_mode==MS_UPFEATURE_SCALAR?"beta+scalar-coverage":"beta-only",F,H,I,ih->n_units,train.size(),val.size(),test.size());
  ...
  for(uint32_t ui=0;ui<ih->n_units;++ui){if(!selected[ui])continue;const MsuiUnit&u=units[ui];bool pure=(u.flags&1)!=0,pna=(u.flags&2)!=0,direct=c->mixed_direct&&!pure;int R=direct?0:(pure?(int)c->pure_bottleneck:(int)c->mixed_bottleneck),O=u.cpg_count;char path[4096];ckpath(path,sizeof(path),c->work_dir,ui);std::vector<float>best;float bestmae=NAN;uint32_t beststep=0;
    ...
    if(load_checkpoint(path,ui,isum,runsum,direct?0:1,R,c->activation,O,I,best,bestmae,beststep)){++resumed;unit_mae[ui]=bestmae;unit_step[ui]=beststep;
      /* Test MAE is recomputed from the loaded weights -- same rows as the
       * fresh path (seed 9001+ui), so resumed and freshly trained units are
       * comparable. It used to be skipped, leaving NaN in pilot.tsv. */
      if(!test.empty()){std::vector<EvalRow>tv=make_eval(h,data.p,cpg,u,test,c->eval_rows,c->batch,c->seed+9001+ui);
        if(!tv.empty()){std::vector<float>bias(O);for(int o=0;o<O;++o)bias[o]=all_bias[cpg[u.output_offset+o]];
          Net net=make_net(direct,I,R,O,bias,c->seed^((uint64_t)ui<<32));unflatten(net,best);
          unit_test_mae[ui]=(float)eval(bh,net,w,dX,tv,c->activation);
          for(const auto&er:tv)unit_test_n[ui]+=er.id.size();destroy(net);}}
      ++processed;continue;}
  ...
  /* A subset model renumbers its units, so the membership table -- which
   * names a unit by INDEX -- must drop the entries whose unit is gone and
   * renumber the rest, or they point past the directory. */
  std::vector<uint32_t>unit_remap(ih->n_units,UINT32_MAX);
  {uint32_t nn=0;for(uint32_t ui=0;ui<ih->n_units;++ui)if(selected[ui])unit_remap[ui]=nn++;}
  std::vector<ms_updec2_membership_t>md;md.reserve(ih->n_real_memberships);
  for(uint32_t q=0;q<ih->n_real_memberships;++q){uint32_t nu=members[q].unit<ih->n_units?unit_remap[members[q].unit]:UINT32_MAX;if(nu==UINT32_MAX)continue;ms_updec2_membership_t m;m.pattern_key=members[q].pattern_key;m.output_offset=members[q].output_offset;m.count=members[q].count;m.unit=nu;md.push_back(m);}
  uint64_t metadata_bytes=sizeof(ms_updec2_header_t)+(uint64_t)F*8+(uint64_t)selected_count*sizeof(ms_updec2_unit_t)+ih->n_cpg*4+(uint64_t)md.size()*sizeof(ms_updec2_membership_t);
  std::vector<ms_updec2_unit_t>od;od.reserve(selected_count);uint64_t po=metadata_bytes+trunk_floats*4;
  for(uint32_t ui=0;ui<ih->n_units;++ui){if(!selected[ui])continue;const MsuiUnit&u=units[ui];bool direct=c->mixed_direct&&!(u.flags&1);uint32_t R=direct?0:((u.flags&1)?c->pure_bottleneck:c->mixed_bottleneck);uint64_t nf=direct?(uint64_t)u.cpg_count*I+u.cpg_count:(uint64_t)R*I+R+(uint64_t)u.cpg_count*R+u.cpg_count;od.push_back({u.output_offset,po,nf*4,u.cpg_count,u.membership_count,(uint16_t)(direct?0:1),(uint16_t)R,u.flags});po+=nf*4;}
  ms_updec2_header_t oh={};memcpy(oh.magic,MS_UPDEC2_MAGIC,8);oh.version=3;oh.flags=MS_UPDEC2_FLAG_GENOMIC|(selected_count<ih->n_units?MS_UPDEC2_FLAG_PARTIAL:0u)|(c->feature_mode==MS_UPFEATURE_COUNT?MS_UPDEC2_FLAG_COUNT:0)|(c->feature_mode==MS_UPFEATURE_BETA?MS_UPDEC2_FLAG_BETA_ONLY:0)|(c->feature_mode==MS_UPFEATURE_SCALAR?MS_UPDEC2_FLAG_SCALAR_COV:0)|(th?MS_UPDEC2_FLAG_TRUNK:0);oh.patterns=P;oh.input_dim=F;oh.n_units=selected_count;oh.n_memberships=(uint32_t)md.size();oh.target_unit_cpgs=ih->target_unit_cpgs;oh.activation=c->activation;oh.n_cpg=ih->n_cpg;oh.mean_offset=sizeof(oh);oh.scale_offset=oh.mean_offset+(uint64_t)F*4;oh.unit_offset=oh.scale_offset+(uint64_t)F*4;oh.cpg_offset=oh.unit_offset+(uint64_t)selected_count*sizeof(ms_updec2_unit_t);oh.membership_offset=oh.cpg_offset+ih->n_cpg*4;oh.param_offset=metadata_bytes;oh.file_bytes=po;oh.index_checksum=isum;oh.trunk_dim=H;
  FILE*out=fopen(c->model_path,"wb");if(!out)diep("cannot create UPDEC2",c->model_path);writeall(out,&oh,sizeof(oh),c->model_path);writeall(out,mean.data(),(size_t)F*4,c->model_path);writeall(out,scale.data(),(size_t)F*4,c->model_path);writeall(out,od.data(),od.size()*sizeof(od[0]),c->model_path);writeall(out,cpg,ih->n_cpg*4,c->model_path);writeall(out,md.data(),md.size()*sizeof(ms_updec2_membership_t),c->model_path);
  uint64_t psum=UINT64_C(1469598103934665603);if(th){writeall(out,trunk_par,(size_t)trunk_floats*4,c->model_path);psum=fnv(psum,trunk_par,(size_t)trunk_floats*4);}for(uint32_t ui=0;ui<ih->n_units;++ui){if(!selected[ui])continue;char path[4096];ckpath(path,sizeof(path),c->work_dir,ui);FILE*f=fopen(path,"rb");if(!f)diep("missing unit checkpoint",path);CheckHeader ch;if(fread(&ch,1,sizeof(ch),f)!=sizeof(ch))diep("truncated unit checkpoint",path);std::vector<float>p(ch.param_floats);if(fread(p.data(),4,p.size(),f)!=p.size())diep("truncated unit checkpoint",path);fclose(f);writeall(out,p.data(),p.size()*4,c->model_path);psum=fnv(psum,p.data(),p.size()*4);}oh.parameter_checksum=psum;if(fseeko(out,0,SEEK_SET)||fwrite(&oh,1,sizeof(oh),out)!=sizeof(oh)||fclose(out))diep("cannot finalize UPDEC2",c->model_path);
