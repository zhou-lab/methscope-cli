...
extern "C" int ms_upunit_train_cuda(const ms_upunit_config_t*c){
  ...
  Work w={df(std::max(c->pure_bottleneck,c->mixed_bottleneck)),df(std::max(c->pure_bottleneck,c->mixed_bottleneck)),df(c->batch),du(c->batch),NULL};CU(cudaMalloc((void**)&w.met,sizeof(Metric)));
