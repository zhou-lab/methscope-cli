...
extern "C" int ms_upunit_train_cuda(const ms_upunit_config_t*c){
  ...
  runsum=fnv(runsum,&c->adaptive_rank,sizeof(c->adaptive_rank));runsum=fnv(runsum,&c->max_rank,sizeof(c->max_rank));runsum=fnv(runsum,&c->homogeneous_rank,sizeof(c->homogeneous_rank));runsum=fnv(runsum,&c->large_cpgs,sizeof(c->large_cpgs));runsum=fnv(runsum,&c->medium_cpgs,sizeof(c->medium_cpgs));runsum=fnv(runsum,&c->var_floor,sizeof(c->var_floor));
  ...
  uint32_t maxR=std::max(c->pure_bottleneck,c->mixed_bottleneck);if(c->adaptive_rank&&c->max_rank>maxR)maxR=c->max_rank;
  ...
  double bias_t0=now();std::vector<float>all_bias(ih->n_cpg,0.0f);std::vector<float>all_sq(ih->n_cpg,0.0f);std::vector<uint16_t>all_count(ih->n_cpg,0);std::vector<float>all_var(ih->n_cpg,0.0f);
  unsigned nth=std::min(8u,std::max(1u,(unsigned)std::thread::hardware_concurrency()));std::vector<std::thread>workers;
  for(unsigned ti=0;ti<nth;++ti)workers.emplace_back([&,ti](){uint64_t beg=ih->n_cpg*ti/nth,end=ih->n_cpg*(ti+1)/nth;for(uint32_t cell:train){const uint16_t*t=truth+(size_t)cell*h->n_cpg;for(uint64_t pos=beg;pos<end;++pos){uint16_t v=t[pos];if(v!=UINT16_MAX){float b=(float)v/65534.0f;all_bias[pos]+=b;all_sq[pos]+=b*b;all_count[pos]++;}}}for(uint64_t pos=beg;pos<end;++pos){uint16_t n=all_count[pos];double mean=n?all_bias[pos]/n:.5;double var=n>1?(double)all_sq[pos]/n-mean*mean:0.0;all_var[pos]=(float)(var>0?var:0.0);double p=std::max(.01,std::min(.99,mean));all_bias[pos]=std::log(p/(1-p));}});
  for(auto&t:workers)t.join();all_count.clear();all_count.shrink_to_fit();all_sq.clear();all_sq.shrink_to_fit();std::fprintf(stderr,"[methscope] upscale-train: prepared genome-wide train-cell biases + variance with %u CPU workers in %.1fs\n",nth,now()-bias_t0);
  /* Complexity-aware per-unit rank: quantitatively flat units (all-0/all-1
   * pattern, or per-CpG variance below var_floor) collapse to homogeneous_rank;
   * large variable units are promoted toward max_rank. */
  std::vector<uint32_t>unit_rank(ih->n_units);std::vector<float>unit_var(ih->n_units,0.0f);
  uint64_t all1key=0;for(uint32_t i=0;i<ih->pattern_length;++i)all1key=all1key*3+1;
  for(uint32_t ui=0;ui<ih->n_units;++ui){const MsuiUnit&u=units[ui];bool pure=(u.flags&1)!=0,pna=(u.flags&2)!=0,direct=c->mixed_direct&&!pure;double s=0;for(uint32_t o=0;o<u.cpg_count;++o)s+=all_var[cpg[u.output_offset+o]];float v=u.cpg_count?(float)(s/u.cpg_count):0.0f;unit_var[ui]=v;uint32_t base=pna?c->mixed_bottleneck:(pure?c->pure_bottleneck:c->mixed_bottleneck);if(direct){unit_rank[ui]=0;continue;}uint32_t R=base;if(c->adaptive_rank){bool homog=!pna&&pure&&u.membership_count==1&&u.first_membership<ih->n_real_memberships&&(members[u.first_membership].pattern_key==0||members[u.first_membership].pattern_key==all1key);if(homog||v<(float)c->var_floor)R=c->homogeneous_rank;else{R=base;if(u.cpg_count>=c->medium_cpgs&&R<32)R=32;if(u.cpg_count>=c->large_cpgs)R=c->max_rank;}if(R>c->max_rank)R=c->max_rank;if(R<1)R=1;}unit_rank[ui]=R;}
  all_var.clear();all_var.shrink_to_fit();
  ...
  if(c->pilot_units_path){char pp[4096];if(std::snprintf(pp,sizeof(pp),"%s/pilot.tsv",c->work_dir)>=(int)sizeof(pp))die("pilot path too long");FILE*f=fopen(pp,"w");if(!f)diep("cannot create pilot metrics",pp);std::fprintf(f,"unit\tclass\tcpgs\tmemberships\tmode\tbottleneck_dim\tvariance\tbest_step\tvalidation_n\tvalidation_mae\n");for(uint32_t ui=0;ui<ih->n_units;++ui)if(selected[ui]){const MsuiUnit&u=units[ui];bool direct=c->mixed_direct&&!(u.flags&1);std::fprintf(f,"%u\t%s\t%u\t%u\t%s\t%u\t%.6g\t%u\t%llu\t%.9g\n",ui,(u.flags&2)?"PNA":(u.flags&1)?"pure":"mixed",u.cpg_count,u.membership_count,direct?"direct":"factor",direct?0:unit_rank[ui],unit_var[ui],unit_step[ui],(unsigned long long)unit_n[ui],unit_mae[ui]);}if(fclose(f))diep("cannot close pilot metrics",pp);std::fprintf(stderr,"[methscope] upscale-train: wrote pilot metrics %s\n",pp);cudaFree(w.z);cudaFree(w.dz);cudaFree(w.y);cudaFree(w.id);cudaFree(w.met);cudaFree(dX);BL(cublasDestroy(bh));if(th)unmap(trunk);unmap(idx);unmap(data);return 2;}
